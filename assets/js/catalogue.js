window.addEventListener("load", function (e) {
  ...
  const usecaseFilter = document.querySelector(
    'select[name="catalogue-usecase"]'
  );
  const providerFilter = document.querySelector(
    'select[name="catalogue-providers"]'
  );
  const stateFilter = document.querySelector(
    'select[name="catalogue-openstate"]'
  );
  ...
  function toggleNonMarkedPanels() {
    ...
    for (let i = 0; i < categories.length; i++) {
      ...
      if (
        type === "0" ||
        type === categories[i].getAttribute("data-category")
      ) {
        ...
        for (let j = 0; j < panels.length; j++) {
          ...
          if (shouldHide) {
            ...
            if (
              !searchInput.value ||
              (searchInput.value !== "" &&
                panels[j].querySelector("mark") !== null)
            ) {
              panels[j].classList.remove("hidden");
  ...
  function fetchAvailability(endpoint) {
    ...
    fetch(
      "https://dashboard.entreprise.api.gouv.fr/api/watchdoge/stats/provider_availabilities?period=6M&endpoint=api/" +
        endpointMatching[endpoint].availability
    )
      .then((response) => {
        if (response.ok) {
          return response.json();
      ...
      .then((data) => {
        if (!data.error) {
          ...
          for (let i = 0; i < spots.length; i++) {
            spots[i].classList.add(rateClass);
          }
          for (let i = 0; i < spots.length; i++) {
            rates[i].innerHTML = availabilityRate.toString() + "%";
          }
          panel.querySelector(".call-count").innerHTML = (
            Math.round(callCount / 100) * 100
          ).toLocaleString("fr-FR");
          panel.querySelector(".fd-errors").innerHTML =
            errorRate.toString() + "%";
          ...
          panel.querySelector(".tab--availability").addEventListener(
            "click",
            function () {
              buildChart(endpoint, dataset);
            },
  ...
  function buildDataset(data, endpoint) {
    ...
    for (let key in data.days_availability) {
      ...
      if (!data.days_availability[key].total) {
        ...
        daily = (
          ((data.days_availability[key].total -
            extractErrorsCountForDay(data.days_availability[key], endpoint)) /
            data.days_availability[key].total) *
          100
        ).toFixed(3);
      }
