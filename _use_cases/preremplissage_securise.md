...
{% asset 'cas-usage/fonctionnement-api-entreprise-cas-usage-preremplissage-securise.png' %}
...
Selon votre cas d'usage spécifique, veillez à **demander uniquement les données nécessaires** dans le cadre du pré-remplissage d'un formulaire disponible après authentification et à fournir les documents nécessaires pour justifier votre [cadre juridique]({{ site.baseurl }}/doc/#le-cadre-juridique). Votre raccordement en sera alors grandement accéléré.
...
Vous avez identifié les données nécessaires pour faciliter le remplissage de formulaires à destination des entreprises et/ou associations, disponibles sous authentification ? Le lien ci-dessous vous permet de faire plus facilement votre demande d'accès spécifique à ce cas d'usage : 

  <a class="tpl-button tpl-button--primary" href="https://datapass.api.gouv.fr/api-entreprise?demarche=preremplissage_securise">🔑 Formulaire d'accès "pré-remplissage - accès sécurisé"</a>
